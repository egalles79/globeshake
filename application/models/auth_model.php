...
class auth_model extends CI_Model {
	...
	// Error Message: 'Indirect modification of overloaded property cart_admin_model::$data has no effect'.
	...
	function register_account()
	{
		...
		if ($this->form_validation->run())
		{
			...
			$profile_data = array(
				...
				'upro_country' => $this->input->post('register_country')
			);
			...
			if ($response)
			{
				...
				//$str_mail = urlencode($email);
				//$str_mail = encrypt($email, KEY_ENCRIPT);
				redirect('auth/verifica_mail/'.$response);
	...
	function resend_activation_token()
	{
		...
		{		
			...
			$user_id = $this->flexi_auth->getUserIdFromEmail($this->input->post('activation_token_identity'));
			...
			//redirect('auth/resend_activation_token');
			if ($response) {
				redirect('auth/verifica_mail/'.$user_id);
			}
		}
		else
		{	
	...
	function manual_reset_forgotten_password($user_id, $token)
	{
		...
		if ($this->form_validation->run())
		{
			...
			if ($this->flexi_auth->forgotten_password_complete($user_id, $token, $new_password)) {
				$this->session->set_flashdata('message',null);
				$strEmail = $this->flexi_auth->get_email($user_id);
				$this->flexi_auth->login($strEmail, $new_password, null);
			}
			else {
	...
	function get_email($user_id) {
		return $this->flexi_auth->get_email($user_id);
	}
