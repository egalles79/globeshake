...
class Auth extends controlador {
    ...
    */
    ...
    {   
        ...
        if ($this->input->post('login_user') || isset($_POST['login_user']))
    ...
    function register_account()
    {
        ...
        $data['title'] = lang('account_creation_title');
		$data['description'] = '';        
    ...
    function verifica_mail($userId = null) {
        ...
        if (isset($userId)) { 
            $data['email'] = $this->flexi_auth_model->get_email($userId);
        }

        $data['title'] = lang('activation_email_successful');
        $data['description'] = str_replace('%e', $data['email'], lang('activation_email_content'));
...
// Get any status message that may have been set.
    ...
    function forgotten_password()
    {
        ...
        $MailEnviado = false;
            ...
            if ($MailEnviado== true)  {
                $data['main_template']  = 'users/password_sent';
                $data['title'] = lang('email_forgot_password_successful');
                $data['description'] = lang('email_forgot_password_content');

            }
            else {
                
                $data['main_template']  = 'users/forgot_password';
                $data['title'] = lang('password_forgotten');
                $data['description'] = '';
            }
    ...
    function manual_reset_forgotten_password($user_id = FALSE, $token = FALSE)
    {
        ...
        {
            $this->load->model('auth_model');
            $PasswordUpdated = $this->auth_model->manual_reset_forgotten_password($user_id, $token);           
        }
        ...
        if ($PasswordUpdated) {
            ...
            $data['main_template']  = 'users/forgot_password_update';
        }
        $data['title'] = '';
        $data['description'] = '';
        $this->load->view('main_template', $data);  
