...
class Flexi_auth extends Flexi_auth_lite
{
	...
	public function resend_activation_token($identity) 
	{
		...
		else if ($this->CI->flexi_auth_model->deactivate_user($user_id))
		{
			...
			$dadesUsuari = $this->CI->flexi_auth_model->get_data($user_id);
	...
	public function forgotten_password($identifier) 
	{
		...
		if ($this->CI->flexi_auth_model->forgotten_password($identity))
		{
			...
			$email_title = ' - Instrucciones para restablecer la contraseña';
	...
	{
		return $this->CI->flexi_auth_model->get_email($user_id);
	}

	public function get_data($user_id = FALSE) {
		return $this->CI->flexi_auth_model->get_data($user_id);
	}

	public function getUserIdFromEmail($email) {
		return $this->CI->flexi_auth_model->get_UserId($email);
	}
